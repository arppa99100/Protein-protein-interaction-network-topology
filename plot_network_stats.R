# This script assumes a tab dilimted file with the following columns:
#	1. Year
#	2. Interactions
#	3. Transitivity
#	4. Characteristic path length (CPL)
#	5. Assortativity
...
network_stats_single_plot <- function (stats) {
	...
	stat_plot <- ggplot(data=stats,aes_string(x="Year",y=measure)) + geom_line(aes(),colour=colors[3]) + 
		geom_point(aes(),colour=colors[3]) + theme(panel.border=element_rect(colour="black",fill=NA),
		panel.grid.minor=element_blank(),panel.grid.major=element_blank(),panel.background=element_blank(),
		legend.position="none",axis.text=element_text(size=8)) + xlab("") + scale_x_continuous(breaks=seq(2000,2014,2))
